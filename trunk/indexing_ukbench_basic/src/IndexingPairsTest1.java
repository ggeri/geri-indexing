...
private static final int NUM_RUNS = 2550;
	...
	{	
		 ...
		 * to .key files and finally reads these files and creates serialized javaobjects on the disk.
		 */
	    ...
	    if(train)
	    {
			...
			try
			{
				...
				String trainedTreeName = "trainedTreePairsSpatialRelat.obj"; 
		...
		if(populate)
		{
			...
			try
			{
				...
				{					
					...
					//String objFileName = KeypointPairsExtraction.imagePathNoBackupMeaningObj + 
						//"ukbench" + formatter.format(image) + ".pair.obj";
			...
			try
			{
				...
				String populateDataSetName = "dataForTreePopulatingPairsSpatialRelat.obj";	
				...
				String imageIndecesName = "imageIndecesPairsSpatialRelat.obj";
		...
		}else
		{
			System.out.println(stars + "\nReading the data set for populating the tree from the disk. ");
			  ...
			  {					
					// imageIDs   	
					//NumberFormat formatter = new DecimalFormat("00000");
					//String objFileName = KeypointPairsExtraction.imagePathNoBackupMeaningObj + "ukbench" + formatter.format(image) + ".pair.obj";
		...
		System.out.println("Invalid image IDs before populating: ");
		int numFailedPts = 0;
		for(int i = 0; i < imageIDs.length; i++)
		{
			if(imageIDs[i] < 0 || imageIDs[i] > 10199)
			{
				System.out.println("loc = " + i + " , imageID = " + imageIDs[i]);
				numFailedPts++;
			}
		}
		System.out.println("Number of failed points = " + numFailedPts);
		
		PopulatedKDTree populatedTree = kdTreeInstance.populateKDTree(trainedTree, dataSetPopulating, imageIDs, imageIndeces, counts);																			
		
//		System.out.println("Invalid image IDs after populating: ");
//		int numFailedPts = 0;
//		for(int i = 0; i< populatedTree.getImageIDs().length; i++)
//		{
//			if(populatedTree.getImageIDs()[i] < 0 || populatedTree.getImageIDs()[i] > 10199)
//			{				
//				System.out.println("loc = " + i + " , imageID = " + populatedTree.getImageIDs()[i] + ", binID " + 
//							populatedTree.getBinIDs()[i]);
//				numFailedPts++;
//				
//			}
//		}
//		System.out.println("Number of points = " + (dataSetPopulating.length / Keypoint.DESCRIPTOR_LENGTH));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
		...
		double[] stats = {0, 0, 0, 0,	// average for bins 1-4 for varianceWide
						  0, 0, 0, 0,	// average for bins 1-4 for varianceNarrow
						  0, 0, 0, 0,	// average for bins 1-4 for varianceNarrow normalized
						  0 };			// number of points that voted
		...
		{			
			...
			//String imageFileName = Path.imagePairsObj + "ukbench" + formatter.format(queryImage) + ".pair.obj";
			
			String imageFileName = Path.filePairMix05OBJ_3 + "imagemix" + formatter.format(queryImage) + ".pair.obj";
			
			double[] votes = kdTreeInstance.voteForImageWithWeightsAndDensityWithStats(trainedTree, 
					populatedTree, counts, imageFileName, IndexingPairsTest1.NUM_IMAGES_POPULATING, stats); 				
...
//////////////////////////////			
		...
		double im1AvrgVotes = im1VotesSum / IndexingPairsTest1.NUM_RUNS;
		double im2AvrgVotes = im2VotesSum / IndexingPairsTest1.NUM_RUNS;
		double im3AvrgVotes = im3VotesSum / IndexingPairsTest1.NUM_RUNS;
		...
		double imRestAvrgVotes = imRestSum / (IndexingPairsTest1.NUM_RUNS * (KeypointsExtraction.NUM_IMAGES-4));
