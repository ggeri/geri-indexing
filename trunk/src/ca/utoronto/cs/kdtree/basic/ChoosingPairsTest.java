...
public class ChoosingPairsTest {
	...
	public static void main(String[] args) 
	{
		...
		ChoosingPairsTest cpTest = new ChoosingPairsTest();
		double fractionOfMatchPts = 0;
		double fractionSum = 0;
		int numImages = 1;
		
		// the pairs of images from the same set and average the results	
		for(int i = 0; i < KeypointPairsExtraction.NUM_IMAGES-1; i = i + 4)
		{
			int image1 = i;
			int image2 = i + 1;
		...
		for(int i = 0; i < KeypointPairsExtraction.NUM_IMAGES; i = i + 4)
		{			
			int image3 = i;
			int randNum = rand.nextInt(KeypointPairsExtraction.NUM_IMAGES);
			while(randNum == i || randNum == i + 1 || randNum == i + 2 || randNum == i + 3)
			{
				randNum = rand.nextInt(KeypointPairsExtraction.NUM_IMAGES);
			}
			int image4 = randNum;
	...
	private double numSamePoints(int image1, int image2)
	{
		...
		short[] image1DataSet = KeypointPairsExtraction.getDataSet(image1);
		short[] image2DataSet = KeypointPairsExtraction.getDataSet(image2);
		...
		for(int i = 0; i < image1DataSet.length; i = i + KeypointPair.DESCRIPTOR_LENGTH)
		{
			short[] point1 = new short[KeypointPair.DESCRIPTOR_LENGTH];			
			short[] point2 = new short[KeypointPair.DESCRIPTOR_LENGTH];
			
			//get point 1
			for(int j = 0; j < KeypointPair.DESCRIPTOR_LENGTH; j++)
			{				
			...
			for(int k = 0; k < image2DataSet.length; k = k + KeypointPair.DESCRIPTOR_LENGTH)
			{			
				// get point 2
				for(int l = 0; l < KeypointPair.DESCRIPTOR_LENGTH; l++)
				{				
					point2[l] = image2DataSet[k+l];		
				}
				// compare point 1 and point 2 and if the same, if they are the same, set 'match' to true
				if(this.distance(point1, point2) < ChoosingPairsTest.maxDistance)
				{					
					match = true;
				}
			}		
